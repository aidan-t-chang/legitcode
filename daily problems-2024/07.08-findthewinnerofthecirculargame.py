...
# The friends are sitting in a circle and are numbered from 
# 1 to n in clockwise order. More formally, 
# moving clockwise from the ith friend brings you to the 
# (i+1)th friend for 1 <= i < n, and moving clockwise from the 
...
class Solution:
    def findTheWinner(self, n, k):
        rep = [int(_) for _ in range(1, n+1)]

        increment = 0
        while len(rep) > 1:
            increment += (k-1)
            increment = increment % len(rep)
            rep.remove(rep[increment])
